...
def create_stand():
    ...
    if course not in store.VALID_COURSES:
        course = "outro"
    stand, access_code, error = store.create_stand(name, course)
    if error == "duplicate":
        return jsonify({
            "error": "Já existe um stand cadastrado com esse nome. Se sua equipe já cadastrou, "
                     "use a opção \"Já tenho um stand\" com o código secreto recebido no cadastro."
        }), 409
