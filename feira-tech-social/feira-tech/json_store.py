...
def _empty_data():
    return {"stands": [], "visitas": [], "engajamentos": []}
...
def _finished_visit(data, stand_id, visitor_key):
    visitor_hash = hashlib.sha256(visitor_key.encode()).hexdigest()
    visit = next(
        (
            item for item in reversed(data["visitas"])
            if item["stand_id"] == stand_id
            and item.get("visitor_hash") == visitor_hash
            and item.get("finished_at")
        ),
        None,
    )
    return visit, visitor_hash
...
def register_share(stand_id, visitor_key):
    with _lock:
        data = _load()
        stand = _find_stand(data, stand_id)
        visit, visitor_hash = _finished_visit(data, stand_id, visitor_key)
        if not stand or not visit:
            return False
        data["engajamentos"].append({
            "id": uuid.uuid4().hex,
            "stand_id": stand_id,
            "stand_nome": stand["name"],
            "visitor_hash": visitor_hash,
...
def create_support(stand_id, visitor_key, message):
    with _lock:
        ...
        duplicate = next((
            item for item in data["engajamentos"]
            if item["stand_id"] == stand_id
            and item.get("visitor_hash") == visitor_hash
            and item.get("type") == "support"
        ), None)
        if duplicate:
            return None, "duplicate"
        support = {
        ...
        data["engajamentos"].append(support)
        _save(data)
        return {key: support[key] for key in ("id", "status")}, None
...
def public_wall(limit=24):
    with _lock:
        supports = [
            {
                "id": item["id"], "stand_id": item["stand_id"],
                "stand_name": item["stand_nome"], "message": item["message"],
                "stars": item["stars"], "created_at": item["created_at"],
            }
            for item in _load()["engajamentos"]
            if item.get("type") == "support" and item.get("status") == "approved"
        ]
        return list(reversed(supports[-limit:]))
...
def dashboard(stand_id, access_code):
    with _lock:
        ...
        engagements = [item for item in data["engajamentos"] if item["stand_id"] == stand_id]
        pending_supports = [
            {"id": item["id"], "message": item["message"], "stars": item["stars"], "created_at": item["created_at"]}
            for item in engagements
            if item.get("type") == "support" and item.get("status") == "pending"
        ]
        return {
            ...
            "shares": sum(item.get("type") == "share" for item in engagements),
            "approved_supports": sum(
                item.get("type") == "support" and item.get("status") == "approved"
                for item in engagements
            ),
